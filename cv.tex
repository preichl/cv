% \documentclass{article}
\documentclass{mycv}

\usepackage[margin=2.2cm]{geometry}
\usepackage{paracol,xcolor,lipsum}
\usepackage{graphicx}
\usepackage{fontawesome}
\usepackage[english]{babel}
\usepackage[T1]{fontenc}
%% \usepackage[scale=.9]{tgheros} % or helvet
\usepackage{helvet} % or helvet
\usepackage{multirow}
\usepackage{hyperref}

\hypersetup{hidelinks=true}

\begin{document}
\pagenumbering{gobble}
\urlstyle{sf}
\columnratio{0.3,0.02}
\begin{paracol}{3}

  \begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\linewidth]{img/avatar.png}
%%    \includegraphics[width=2cm]{img/avatar.png}
  \end{figure}

  \vspace{1cm}

  \leftline{
    \begin{tabular}{cl}
      \multirow{2}{*}{\Large{\faMale}} & \Large{Name} \\
      &  Pavel Reichl \\
      \multirow{0}{*}{} & \\
      \multirow{3}{*}{\Large{\faMapMarker}} & \Large{Address} \\
      &  Brno \\
      &  Czech Republic \\
      \multirow{0}{*}{} & \\
      \multirow{1}{*}{\Large{\faEnvelopeSquare}} & \Large{Email} \\
      %% & \href{mailto:me@example.com}{\nolinkurl{me@example.com}} \\
      & \small{\url{reichl.pavel@gmail.com}} \\
      \multirow{0}{*}{} & \\
      \multirow{1}{*}{\Large{\faMobile}} & \Large{Phone} \\
      & +420 777 983 828 \\
      \multirow{0}{*}{} & \\
      \Large{\faLinkedinSquare} & \Large{Linkedin} \\
      \multicolumn{2}{l}{\small{\url{cz.linkedin.com/in/pavelreichl}}} \\
    \end{tabular}
  }

\vspace{10mm}

\topic{Profile}{\faFile}

{\small{
    I am a software engineer interested in system and security software and also more generally in open source. I have a background of game developer which gave me experiece with optimization and computer graphica. I am looking for new chalanges.
  }
}

\vspace{10mm}

\topic{Languages}{\faFlag}
\vspace{2mm}
\leftline{
  \begin{tabular}{rc}
    \textsc{\large{English}} & \multirow{2}{*}{\large{\faStar \faStar \faStarHalfO}} \\ Working Proficiency \\
    \multirow{0}{*}{} & \\
    \textsc{\large{Czech}} & \multirow{2}{*}{\large{\faStar \faStar \faStar}} \\ Native \\
    \multirow{0}{*}{} & \\
    \textsc{\large{German}} & \multirow{2}{*}{\large{\faStarHalfO \faStarO \faStarO}} \\ Elementary Proficiency
  \end{tabular}
}


\switchcolumn
\switchcolumn

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Right Page
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\leftline{
  \resizebox{0.8\linewidth}{!}{\textbf{Pavel Reichl}}
}

\vspace{4mm}

\leftline{
  \resizebox{0.55\linewidth}{!}{\textsc{Software engineer}}
}

\vspace{15mm}

%% Work experience  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\topic{Work Experience}{\faCogs}

\jobls{Oct 2013-–May 2016}{\redhat}{Brno}{Software engineer}{
Designing and developing new features (primarily C) with ephesis on product security and efficiency, bug fixing, testing, helping users and support with troubleshooting, participating in planning and documentation processes.
}{
  C|Git|Fedora|LDAP|Kerberos|PAM|Autotools}

\mjobls{Sep 2011-–Jan 2013}{Seznam.cz}{Brno}{Tech team programmer}{
    Member of tech team responsible for maintenance of corporate technology and development of new tools
}{
  C++|Python|Git}{
  Programmer}{
  Maintenance of high-traffic web applications. (novinky.cz, sport.cz, super.cz)}{
  Python|SVN|Debian}

\jobls{Jun 2008-–Nov 2010}{\hammerware}{Brno}{Programmer}{
    Development of cross-platform games - programming, debugging, optimization and testing.
}{C++|SVN|Ogre}

\jobls{Jun 2007--Oct 2010}{Handjoy}{Brno}{Programmer}{
    One time contract on a single project.
}{C++|OpenGL|SDL}

\topicend

%% Education %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\topic{Education}{\faMortarBoard}

\uni{2010--2013}{Masaryk University}{Brno}{Masters’s degree of Applied Informatics}{Procedural modelling of buildings}

\uni{2005--2008}{Masaryk University}{Brno}{Bachelor’s degree of Applied Informatics}{Octree-based Ray Tracer}

\topicend

\topic{Projects}{\faFolder}
\project{SSSD}{\redhat}{
  SSSD is a system daemon. Its primary function is to provide access to identity and authentication remote resource.}{\faLock}
\project{pam\_hbac}{\redhat}{
  A PAM account module that evaluates HBAC rules stored on an IPA server.}{\faLock}
\project{Family Farm}{\hammerware}{
  Innovative sim/tycoon game in the setting of 19th century farmstead for PC, Mac and
  Linux.}{\faGamepad}

\end{paracol}

\end{document}
